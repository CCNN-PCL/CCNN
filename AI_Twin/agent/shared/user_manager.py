...
class UserProfileManager:
    ...
    def _init_database(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.info("用户资料数据库初始化完成")
            
        except Exception as e:
            logger.error(f"用户资料数据库初始化失败: {str(e)}")
            raise
    ...
    def create_user_profile(self, profile: UserProfile) -> bool:
        ...
        try:
            ...
            c.execute('''
                INSERT INTO user_profiles (user_id, full_name, avatar, phone, address, 
                                         birth_date, gender, emergency_contact, medical_conditions,
                                         allergies, medications, preferences)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                profile.user_id,
                profile.full_name,
        ...
        try:
            ...
            c.execute('''
                INSERT INTO user_preferences (user_id, language, timezone, theme, 
                                            notifications, privacy_settings, display_settings)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                preferences.user_id,
    ...
    def update_user_preferences(self, preferences: UserPreferences) -> bool:
        ...
        try:
            ...
            ''', (
                ...
                json.dumps(preferences.privacy_settings),
                json.dumps(preferences.display_settings),
                datetime.now(), preferences.user_id
            ))
...
class UserDataManager:
    ...
    def _init_database(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.info("用户数据数据库初始化完成")
            
        except Exception as e:
            logger.error(f"用户数据数据库初始化失败: {str(e)}")
            raise
