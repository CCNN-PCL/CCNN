...
class BaseAgent(ABC):
    ...
    def _init_llm_caller(self):
        """初始化LLM调用器"""
        try:
            from shared.llm_caller import call_llm as original_call_llm
            # 包装call_llm以传递智能体名称
    ...
    async def safe_execute(self, *args, **kwargs) -> Any:
        ...
        try:
            # 使用print确保输出到终端
            print("\n" + "🤖" * 50)
            print(f"🤖 智能体开始执行: {self.agent_name}")
