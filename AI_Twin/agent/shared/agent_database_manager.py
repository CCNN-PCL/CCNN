...
from auth_manager import authz_manager, audit_manager, User, UserRole
from database_auth import db_auth_manager, OperationType, DataAccessLevel
...
class AgentDatabaseManager:
    ...
    def _verify_permissions(self, data_type: str, operation: str, target_user_id: str = None) -> Tuple[bool, str]:
        """验证权限"""
        try:
            # 1. 验证智能体访问权限
    ...
    def _log_database_operation(self, operation: str, table_name: str, 
                               target_user_id: str = None, details: str = None):
        """记录数据库操作日志"""
        try:
            # 记录到数据库权限管理器
...
def require_agent_permission(agent_name: str, data_type: str = None, operation: str = "read"):
    """智能体权限验证装饰器"""
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, user: User, *args, **kwargs):
            try:
                # 1. 验证智能体访问权限
                if not authz_manager.check_agent_access(user.role, agent_name, user.user_id):
                    logger.warning(f"用户 {user.username} 无权限访问 {agent_name}")
                    return {"error": f"权限不足，无法访问 {agent_name}"}
                
                # 2. 验证数据访问权限
                if data_type and not authz_manager.check_data_access(user.role, data_type, user.user_id):
                    logger.warning(f"用户 {user.username} 无权限访问 {data_type} 数据")
                    return {"error": f"权限不足，无法访问 {data_type} 数据"}
                
                # 3. 验证操作权限
                if not authz_manager.check_action_permission(user.role, operation, user.user_id):
