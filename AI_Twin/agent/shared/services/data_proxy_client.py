...
class DataProxyClient:
    ...
    def __init__(self, proxy_config: Dict[str, Any]):
        ...
        self.logger.info(f"[{self.__class__.__name__}] 初始化完成 - URL: {self.proxy_url}")
    
    async def request_medical_data(self,
                                  intent_type: str,
                                  specialty: str,
                                  user_id: str,
        ...
        try:
            ...
            # 发送请求
            response = await self._send_request_with_retry("/api/medical-data", request_data)
    ...
    async def _send_request_with_retry(self, endpoint: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """发送请求（带重试机制）"""
        last_exception = None
        
        for attempt in range(self.retry_count):
            try:
                self.logger.info(f"[{self.__class__.__name__}] 发送请求 (尝试 {attempt + 1}/{self.retry_count})")
                response = await self._send_request(endpoint, data)
                return response
                
            except Exception as e:
                last_exception = e
                self.logger.warning(f"[{self.__class__.__name__}] 请求失败 (尝试 {attempt + 1}/{self.retry_count}): {str(e)}")
        ...
        # 所有重试都失败
        raise Exception(f"请求失败，已重试 {self.retry_count} 次: {str(last_exception)}")
    ...
    def _generate_cache_key(self, intent_type: str, specialty: str, user_id: str) -> str:
        """生成缓存键"""
