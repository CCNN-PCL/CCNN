        ...
        try:
            ...
            if self.db_manager.create_user(user, password):
                ...
                self.db_manager.log_audit_event(
                    user_id, "USER_REGISTERED", "user", details=f"username={username}"
                )
    ...
    def update_user_profile(
        ...
        try:
            ...
            if self.db_manager.update_user(user):
                # 记录审计日志
                self.db_manager.log_audit_event(
                    user_id, "PROFILE_UPDATED", "user", details=json.dumps(profile_data)
                )
...
class AuthorizationManager:
    ...
    def check_agent_access(
        self, user_role: UserRole, agent_name: str, user_id: str = None
    ) -> bool:
        """检查智能体访问权限"""
    ...
    def check_data_access(
        self, user_role: UserRole, data_type: str, user_id: str = None
    ) -> bool:
        """检查数据访问权限"""
        if self.use_database_permissions and self.permission_db:
            return self.permission_db.check_permission(
    ...
    def check_action_permission(
        self, user_role: UserRole, action: str, user_id: str = None
    ) -> bool:
        """检查操作权限"""
        if self.use_database_permissions and self.permission_db:
            return self.permission_db.check_permission(
    ...
    def get_user_permissions(
        self, user_role: UserRole, user_id: str = None
    ) -> Dict[str, List[str]]:
        """获取用户权限列表"""
