...
from .auth_manager import User, UserRole, UserStatus, auth_manager, authz_manager
...
class UserProfileManager:
    ...
    def _init_database(self):
        ...
        try:
            # 检查是否使用MySQL
            if db_config and db_config.database_type == 'mysql':
            ...
            async with self._db_manager.pool.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        ...
                        json.dumps(profile.medical_conditions) if profile.medical_conditions else None,
    ...
    def _create_user_profile_sqlite(self, profile: UserProfile) -> bool:
        ...
        try:
            ...
            c.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                profile.user_id, profile.full_name, profile.avatar, profile.phone,
                profile.address, profile.birth_date, profile.gender, profile.emergency_contact,
                json.dumps(profile.medical_conditions), json.dumps(profile.allergies),
                json.dumps(profile.medications), json.dumps(profile.preferences)
            ))
            conn.commit()
            conn.close()
            logger.info(f"用户资料创建成功: {profile.user_id}")
                ...
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    if row:
                        return UserProfile(
                            ...
                            medical_conditions=json.loads(row['medical_conditions']) if row.get('medical_conditions') else [],
    ...
    def _get_user_profile_sqlite(self, user_id: str) -> Optional[UserProfile]:
        ...
        try:
            ...
            if row:
                return UserProfile(
                    user_id=row[0], full_name=row[1], avatar=row[2], phone=row[3],
                    address=row[4], birth_date=row[5], gender=row[6], emergency_contact=row[7],
            ...
            async with self._db_manager.pool.get_connection() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
                        ...
                        WHERE user_id = %s
                    """, (
                        profile.full_name, profile.avatar, profile.phone, profile.address,
                        profile.birth_date, profile.gender, profile.emergency_contact,
                        json.dumps(profile.medical_conditions) if profile.medical_conditions else None,
                        json.dumps(profile.allergies) if profile.allergies else None,
                        json.dumps(profile.medications) if profile.medications else None,
                        json.dumps(profile.preferences) if profile.preferences else None,
    ...
    def upload_avatar(self, user_id: str, image_data: bytes) -> Tuple[bool, str, Optional[str]]:
        """上传用户头像"""
        try:
            # 验证图片格式和大小
            ...
            async with self._db_manager.pool.get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        ...
                        activity.timestamp, activity.ip_address, activity.user_agent,
    ...
    async def _get_user_activities_mysql(self, user_id: str, limit: int) -> List[UserActivity]:
        """使用MySQL获取用户活动记录"""
        try:
            import aiomysql
            async with self._db_manager.pool.get_connection() as conn:
    ...
    def _get_user_activities_sqlite(self, user_id: str, limit: int) -> List[UserActivity]:
        ...
        try:
            ...
            return [
                UserActivity(
                    user_id=row[0], activity_type=row[1], description=row[2],
                    timestamp=datetime.fromisoformat(row[3]) if isinstance(row[3], str) else row[3],
                    ip_address=row[4], user_agent=row[5],
...
class UserPreferencesManager:
    ...
    async def _create_user_preferences_mysql(self, preferences: UserPreferences) -> bool:
        """使用MySQL创建用户偏好设置"""
        try:
            import aiomysql
            async with self._db_manager.pool.get_connection() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
                        ...
                        ON DUPLICATE KEY UPDATE
                            ...
                            updated_at = CURRENT_TIMESTAMP
                    """, (
                        preferences.user_id, preferences.language, preferences.timezone, preferences.theme,
                        json.dumps(preferences.notifications) if preferences.notifications else None,
                        json.dumps(preferences.privacy_settings) if preferences.privacy_settings else None,
                        json.dumps(preferences.display_settings) if preferences.display_settings else None
                    ))
                    await conn.commit()
    ...
    def _create_user_preferences_sqlite(self, preferences: UserPreferences) -> bool:
        ...
        try:
            ...
            ''', (
                preferences.user_id, preferences.language, preferences.timezone, preferences.theme,
                json.dumps(preferences.notifications), json.dumps(preferences.privacy_settings),
    ...
    async def _get_user_preferences_mysql(self, user_id: str) -> Optional[UserPreferences]:
        """使用MySQL获取用户偏好设置"""
        try:
            import aiomysql
            async with self._db_manager.pool.get_connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    if row:
                        return UserPreferences(
                            ...
                            notifications=json.loads(row['notifications']) if row.get('notifications') else {},
                            privacy_settings=json.loads(row['privacy_settings']) if row.get('privacy_settings') else {},
                            display_settings=json.loads(row['display_settings']) if row.get('display_settings') else {}
                        )
                    return None
        except Exception as e:
    ...
    def _get_user_preferences_sqlite(self, user_id: str) -> Optional[UserPreferences]:
        ...
        try:
            ...
            if row:
                return UserPreferences(
                    user_id=row[0], language=row[1], timezone=row[2], theme=row[3],
    ...
    async def _update_user_preferences_mysql(self, preferences: UserPreferences) -> bool:
        """使用MySQL更新用户偏好设置"""
        try:
            import aiomysql
            async with self._db_manager.pool.get_connection() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
                        ...
                        WHERE user_id = %s
                    """, (
                        preferences.language, preferences.timezone, preferences.theme,
                        json.dumps(preferences.notifications) if preferences.notifications else None,
                        json.dumps(preferences.privacy_settings) if preferences.privacy_settings else None,
                        json.dumps(preferences.display_settings) if preferences.display_settings else None,
                        preferences.user_id
    ...
    def _update_user_preferences_sqlite(self, preferences: UserPreferences) -> bool:
        ...
        try:
            ...
            ''', (
                preferences.language, preferences.timezone, preferences.theme,
                json.dumps(preferences.notifications), json.dumps(preferences.privacy_settings),
                json.dumps(preferences.display_settings), datetime.now(), preferences.user_id
            ))
            conn.commit()
            conn.close()
            logger.info(f"用户偏好设置更新成功: {preferences.user_id}")
...
class UserDataManager:
                ...
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    if row:
                        return {
                            ...
                            'total_medical_images': row.get('total_medical_images', 0),
                            'total_medical_records': row.get('total_medical_records', 0),
                            'last_activity': str(row['last_activity']) if row.get('last_activity') else None,
                            'data_usage_mb': float(row.get('data_usage_mb', 0.0))
                        }
            return {
    ...
    def _get_user_data_stats_sqlite(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if row:
                return {
                    'total_chat_messages': row[0], 'total_medical_images': row[1],
                    'total_medical_records': row[2], 'last_activity': row[3],
                    'data_usage_mb': row[4]
                }
            return {
                'total_chat_messages': 0, 'total_medical_images': 0,
                'total_medical_records': 0, 'last_activity': None, 'data_usage_mb': 0.0
            }
        except Exception as e:
            logger.error(f"获取用户数据统计失败: {str(e)}")
            return {}
    ...
    async def _update_user_data_stats_mysql(self, user_id: str, stats: Dict[str, Any]) -> bool:
        """使用MySQL更新用户数据统计"""
        try:
            import aiomysql
            async with self._db_manager.pool.get_connection() as conn:
    ...
    def _update_user_data_stats_sqlite(self, user_id: str, stats: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            c.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                user_id, stats.get('total_chat_messages', 0),
                stats.get('total_medical_images', 0), stats.get('total_medical_records', 0),
                stats.get('last_activity'), stats.get('data_usage_mb', 0.0), datetime.now()
            ))
            conn.commit()
            conn.close()
            return True
...
class UserManager:
    ...
    def create_user_profile(self, user_id: str, profile_data: Dict[str, Any]) -> Tuple[bool, str]:
        """创建用户资料"""
        try:
            profile = UserProfile(
                user_id=user_id,
    ...
    def upload_user_avatar(self, user_id: str, image_data: bytes) -> Tuple[bool, str, Optional[str]]:
        """上传用户头像"""
    ...
    def update_user_preferences(self, user_id: str, preferences_data: Dict[str, Any]) -> Tuple[bool, str]:
        """更新用户偏好设置"""
        try:
    ...
    def record_user_activity(self, user_id: str, activity_type: str, description: str, 
                           ip_address: str = None, user_agent: str = None, metadata: Dict[str, Any] = None) -> bool:
        """记录用户活动"""
...
__all__ = [
    ...
    'UserProfileManager', 'UserPreferencesManager', 'UserDataManager', 'UserManager',
    'user_manager'
]
