...
# MCP协议支持
try:
    from shared.agents.utils.mcp_storage_client import MCPStorageClient
...
class DatabaseStorageConfig:
    """数据库存储服务配置"""
    beijing_url: str = os.getenv("DATABASE_STORAGE_BEIJING_URL", "http://database-storage-beijing:8000")
    shanghai_url: str = os.getenv("DATABASE_STORAGE_SHANGHAI_URL", "http://database-storage-shanghai:8000")
    timeout: int = int(os.getenv("DATABASE_STORAGE_TIMEOUT", "30"))
    retry_count: int = 2
    enable_logging: bool = True
    # MCP协议配置
    use_mcp: bool = os.getenv("USE_MCP_PROTOCOL", "false").lower() == "true"
...
class DatabaseStorageClient:
    ...
    def __init__(self, config: DatabaseStorageConfig):
        ...
        if self.config.use_mcp and MCP_STORAGE_AVAILABLE:
            try:
                ...
                self.logger.warning(f"[DatabaseStorageClient] MCP客户端初始化失败: {e}，将使用HTTP协议")
                self.mcp_client = None
        elif self.config.use_mcp and not MCP_STORAGE_AVAILABLE:
            self.logger.warning("[DatabaseStorageClient] MCP协议已配置但库不可用，将使用HTTP协议")
    ...
    async def __aenter__(self):
        ...
        if self.mcp_client:
            ...
            self.logger.info("[DatabaseStorageClient] MCP协议已启用，将在获取access_token后初始化连接")
        else:
    ...
    async def retrieve_medical_data(
        ...
        if not data_addresses:
            self.logger.info(f"[DatabaseStorageClient] 没有数据地址，返回空数据")
            print(f"\n[数据存储服务] 没有数据地址，返回空数据\n")
            return {
        ...
        print(f"\n[数据存储服务] ========================================")
        print(f"  数据读取完成，合并结果:")
        print(f"    - 病史数据: {len(merged_data.get('medical_history', []))} 条" if isinstance(merged_data.get('medical_history'), list) else f"    - 病史数据: {'有' if merged_data.get('medical_history') else '无'}")
        print(f"    - 用药记录: {len(merged_data.get('medications', []))} 条" if isinstance(merged_data.get('medications'), list) else f"    - 用药记录: {'有' if merged_data.get('medications') else '无'}")
        print(f"    - 化验报告: {len(merged_data.get('lab_results', []))} 条" if isinstance(merged_data.get('lab_results'), list) else f"    - 化验报告: {'有' if merged_data.get('lab_results') else '无'}")
        print(f"    - 影像数据: {len(merged_data.get('imaging_data', []))} 条" if isinstance(merged_data.get('imaging_data'), list) else f"    - 影像数据: {'有' if merged_data.get('imaging_data') else '无'}")
        print(f"    - 手术记录: {len(merged_data.get('surgical_records', []))} 条" if isinstance(merged_data.get('surgical_records'), list) else f"    - 手术记录: {'有' if merged_data.get('surgical_records') else '无'}")
        ...
        try:
            ...
            print(f"数据键: {list(merged_data.keys()) if isinstance(merged_data, dict) else 'N/A'}")
        print("=" * 80 + "\n")
        
        return merged_data
    
    def _group_by_location(self, data_addresses: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
        按地域分组数据地址
    ...
    async def _retrieve_from_location(
        ...
        try:
            self.logger.info(f"[DatabaseStorageClient] 从{location}读取数据，地址数量: {len(data_addresses)}")
            ...
            for i, addr in enumerate(data_addresses, 1):
                ...
                print(f"       地址: {addr.get('address', 'N/A')[:80]}..." if len(addr.get('address', '')) > 80 else f"       地址: {addr.get('address', 'N/A')}")
            ...
            async with self.session.post(url, json=request_data, headers=headers) as response:
                if response.status == 200:
                    ...
                    if result.get("success"):
                        ...
                        self.logger.info(f"[DatabaseStorageClient] {location}数据读取成功")
                        ...
                        print(f"  医疗数据:")
                        if medical_data:
                            print(f"    - 病史数据: {len(medical_data.get('medical_history', []))} 条" if isinstance(medical_data.get('medical_history'), list) else f"    - 病史数据: {'有' if medical_data.get('medical_history') else '无'}")
                            print(f"    - 用药记录: {len(medical_data.get('medications', []))} 条" if isinstance(medical_data.get('medications'), list) else f"    - 用药记录: {'有' if medical_data.get('medications') else '无'}")
                            print(f"    - 化验报告: {len(medical_data.get('lab_results', []))} 条" if isinstance(medical_data.get('lab_results'), list) else f"    - 化验报告: {'有' if medical_data.get('lab_results') else '无'}")
                            print(f"    - 影像数据: {len(medical_data.get('imaging_data', []))} 条" if isinstance(medical_data.get('imaging_data'), list) else f"    - 影像数据: {'有' if medical_data.get('imaging_data') else '无'}")
                            print(f"    - 手术记录: {len(medical_data.get('surgical_records', []))} 条" if isinstance(medical_data.get('surgical_records'), list) else f"    - 手术记录: {'有' if medical_data.get('surgical_records') else '无'}")
                            ...
                            if medical_data.get('medical_history'):
                                ...
                                if isinstance(history, list) and len(history) > 0:
                                    ...
                                    if isinstance(first_item, dict):
                                        for key in list(first_item.keys())[:5]:  # 只打印前5个字段
                        ...
                        self.logger.warning(f"[DatabaseStorageClient] {location}数据读取失败: {result.get('message')}")
                    ...
                    self.logger.error(f"[DatabaseStorageClient] {location}HTTP错误: {response.status} - {error_text}")
    ...
    def _merge_results(self, results: List[Any]) -> Dict[str, Any]:
        ...
        for result in results:
            if isinstance(result, Exception):
                self.logger.warning(f"[DatabaseStorageClient] 读取结果包含异常: {str(result)}")
                continue
            ...
            async with session.post(url, json=request_data) as response:
                if response.status == 200:
                    ...
                    if result.get("success"):
                        ...
                        logging.warning(f"[retrieve_medical_data_via_internal_api] 请求失败: {result.get('message')}")
                        return {}
                else:
                    error_text = await response.text()
                    logging.error(f"[retrieve_medical_data_via_internal_api] HTTP错误: {response.status} - {error_text}")
                    return {}
        
        except Exception as e:
            logging.error(f"[retrieve_medical_data_via_internal_api] 调用异常: {str(e)}")
            return {}
