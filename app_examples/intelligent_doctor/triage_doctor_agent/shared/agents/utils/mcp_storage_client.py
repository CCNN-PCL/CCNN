...
try:
    ...
    MCP_AVAILABLE = False
    logging.warning("[MCPStorageClient] MCP库未安装，将无法使用MCP协议")
except (UnicodeDecodeError, Exception) as e:
    # 处理fastmcp初始化时的编码错误或其他异常
    MCP_AVAILABLE = False
    error_msg = str(e)
    if "utf-8" in error_msg.lower() or "codec" in error_msg.lower():
        logging.warning(f"[MCPStorageClient] MCP库初始化失败（编码错误）: {error_msg[:100]}，将无法使用MCP协议")
        logging.warning("[MCPStorageClient] 建议：检查项目目录或父目录中的.env文件编码，确保为UTF-8格式")
        logging.warning("[MCPStorageClient] 或者删除/重命名有问题的.env文件")
        logging.warning("[MCPStorageClient] 临时解决方案：设置环境变量 PYDANTIC_SETTINGS_SOURCE_DOTENV_ENABLED=false")
    else:
        logging.warning(f"[MCPStorageClient] MCP库初始化失败: {error_msg[:100]}，将无法使用MCP协议")
...
class MCPServer:
    ...
    async def initialize(self) -> None:
        ...
        token_preview = self.config.token[:10] + "..." if len(self.config.token) > 10 else self.config.token
        self.logger.info(f"[MCPServer] 使用token: {token_preview} (长度: {len(self.config.token)})")
        ...
        try:
            ...
            if self.config.transport_type == "sse":
                ...
                transport = await self.exit_stack.enter_async_context(sse_client(
                    url=url,
            ...
            if "401" in error_msg or "Unauthorized" in error_msg:
                ...
                self.logger.error("  - 检查 start_backend.ps1 中的 MCP_SERVER_TOKEN 配置")
                self.logger.error("=" * 80)
    ...
    async def execute_tool(
        ...
        while attempt < retries:
            try:
                ...
                self.logger.info(f"[MCPServer] 工具参数摘要: department={arguments.get('department', 'N/A')}, user_id={arguments.get('user_id', 'N/A')}, data_addresses={len(arguments.get('data_addresses', []))}个")
                print(f"\n[MCPServer] 调用MCP工具详情:")
                ...
                print(f"    - data_addresses: {len(arguments.get('data_addresses', []))} 个")
                print(f"    - data_types: {arguments.get('data_types', [])}")
                print(f"    - access_tokens: {len(arguments.get('access_tokens', []))} 个")
                    ...
                    if "content" in result:
                        ...
                        if isinstance(content, list) and len(content) > 0:
                            ...
                            if isinstance(content[0], dict) and "text" in content[0]:
                                ...
                                text_preview = str(text_data)[:300] if len(str(text_data)) > 300 else str(text_data)
                                print(f"  响应内容预览: {text_preview}")
                                if len(str(text_data)) > 300:
                                    print(f"  ... (内容过长，已截断，总长度: {len(str(text_data))} 字符)")
                    # 打印完整的响应（用于调试）
                    try:
                        import json
                        ...
                        if len(result_json) > 1000:
                            ...
                            print(f"  ... (响应过长，已截断，总长度: {len(result_json)} 字符)")
                        else:
                            print(f"\n  完整响应内容:")
                            print(result_json)
                    except Exception as e:
                ...
                attempt += 1
                self.logger.warning(f"[MCPServer] 工具执行失败 (尝试 {attempt}/{retries}): {e}")
                if attempt < retries:
...
class MCPStorageClient:
    ...
    def _initialize_mcp_servers(self) -> None:
        ...
        if beijing_url:
            ...
            self.logger.info(f"[MCPStorageClient] 预创建北京MCP服务器连接: {beijing_url}")
        ...
        if shanghai_url:
            ...
            self.logger.info(f"[MCPStorageClient] 预创建上海MCP服务器连接: {shanghai_url}")
        
        self.logger.info(f"[MCPStorageClient] 已预配置 {len(self.mcp_servers)} 个MCP服务器（从环境变量）")
    ...
    async def initialize(self) -> None:
        ...
        for server_key, server in self.mcp_servers.items():
            location, server_url = server_key if isinstance(server_key, tuple) else (server_key, "")
            if not server.config.token or server.config.token == "test":
                has_default_token = True
                self.logger.info(f"[MCPStorageClient] {location} - {server_url} 使用默认token，将延迟初始化（等待access_token）")
                print(f"[MCPStorageClient] ⚠️  {location} - {server_url} 使用默认token，将延迟初始化（等待从数据地址获取access_token）")
        
        if has_default_token:
            self.logger.info("[MCPStorageClient] 检测到默认token，跳过立即初始化，将在获取access_token后初始化")
            print("[MCPStorageClient] ✅ 延迟初始化：等待从数据代理应用获取access_token后再连接MCP服务器")
            return
        ...
        for server_key, server in self.mcp_servers.items():
            ...
            try:
                ...
                self.logger.info(f"[MCPStorageClient] {location} - {server_url} MCP服务器初始化成功")
            except Exception as e:
                self.logger.error(f"[MCPStorageClient] {location} - {server_url} MCP服务器初始化失败: {e}")
                # ⚠️ 不立即抛出异常，允许后续使用access_token重试
                self.logger.warning(f"[MCPStorageClient] {location} - {server_url} MCP服务器初始化失败，将在获取access_token后重试")
    ...
    async def cleanup(self) -> None:
        ...
        for server_key, server in self.mcp_servers.items():
            ...
            try:
                ...
                self.logger.info(f"[MCPStorageClient] {location} - {server_url} MCP服务器已清理")
            except Exception as e:
                self.logger.error(f"[MCPStorageClient] {location} - {server_url} MCP服务器清理失败: {e}")
    ...
    def _extract_department(self, data_address: Dict[str, Any]) -> str:
        ...
        if department and department.strip():
            self.logger.info(f"[MCPStorageClient] 从数据地址中提取到department: {department}")
        ...
        if specialty and specialty in self.specialty_to_department:
            ...
            self.logger.info(f"[MCPStorageClient] 从parameters.specialty提取到科室: {mapped_dept}")
            print(f"[MCPStorageClient] ✅ 从parameters.specialty提取到科室: {mapped_dept}")
            return mapped_dept
        ...
        # 最后：默认返回"内科"
        self.logger.warning(f"[MCPStorageClient] 无法确定科室，使用默认值'内科'")
        self.logger.warning(f"[MCPStorageClient] 数据地址详情: hospital={hospital}, department={department}, parameters={parameters}")
        print(f"[MCPStorageClient] ⚠️  警告: 无法确定科室，使用默认值'内科'")
    ...
    def _build_mcp_arguments(
        ...
        department = self._extract_department(data_addresses[0]) if data_addresses else "内科"
        ...
        for addr in data_addresses:
            ...
            # 记录access_token提取情况
            if not access_token:
                self.logger.warning(f"[MCPStorageClient] 数据地址中缺少access_token: address={address}, data_type={data_type}")
            ...
            if access_token:
                ...
                self.logger.debug(f"[MCPStorageClient] 提取到access_token: {access_token[:20]}... (长度: {len(access_token)})")
            else:
                self.logger.warning(f"[MCPStorageClient] 数据地址中access_token为空: address={address}")
        ...
        self.logger.info(f"[MCPStorageClient] 构建MCP参数: department={department}, user_id={user_id}, request_id={request_id}")
        self.logger.info(f"[MCPStorageClient] 数据地址信息（用于参考）: {len(addresses_info)}个, 数据类型: {list(set(data_types))}, access_tokens: {len(access_tokens)}个")
        return arguments
    ...
    def _parse_mcp_response(self, mcp_result: Any) -> Dict[str, Any]:
            ...
            if hasattr(mcp_result, 'content'):
                ...
                if isinstance(content, list) and len(content) > 0:
                    ...
                    if isinstance(text_data, str):
                        ...
                        # 检测常见的错误消息
                        if "invalid token" in text_lower or "token无效" in text_lower or "认证失败" in text_lower:
                            self.logger.error(f"[MCPStorageClient] ❌ MCP服务器返回认证错误: {text_data}")
                            print(f"\n[MCPStorageClient] ❌ MCP服务器返回认证错误: {text_data}")
                            print(f"  这通常意味着access_token无效或已过期")
                            print(f"  请检查:")
                            print(f"    1. 数据地址中的access_token是否正确")
                            print(f"    2. access_token是否已过期")
                            print(f"    3. MCP服务器是否接受该token")
                    ...
                    try:
                        ...
                        if isinstance(parsed_data, dict):
                            ...
                            if "medical_data" in parsed_data and isinstance(parsed_data["medical_data"], dict):
                                ...
                                # 先更新顶层字段（如果有非null的值）
                                for key in ["medical_history", "medications", "lab_results", "imaging_data", "surgical_records", "health_monitoring"]:
                                    if key in parsed_data and parsed_data[key] is not None:
                                        medical_data[key] = parsed_data[key]
                                    elif key in nested_medical_data and nested_medical_data[key] is not None:
                                ...
                                if "sources" in nested_medical_data:
                                    ...
                                    medical_data["sources"] = sources if isinstance(sources, list) else [sources]
                                # 更新其他字段
                                for key in ["available_data_types", "request_id"]:
                                    if key in parsed_data:
                                        ...
                                        medical_data[key] = nested_medical_data[key]
                                self.logger.info(f"[MCPStorageClient] ✅ 从medical_data嵌套字段中提取医疗数据")
                            else:
                        ...
                        if isinstance(text_data, dict):
                            ...
                            if "medical_data" in text_data and isinstance(text_data["medical_data"], dict):
                            ...
                            self.logger.warning("[MCPStorageClient] MCP响应content不是JSON格式")
                            medical_data["raw_response"] = text_data
                elif isinstance(content, dict):
                    # content直接是字典
                    # ✅ 修复：检查是否有medical_data嵌套字段
                    if "medical_data" in content and isinstance(content["medical_data"], dict):
                    ...
                    self.logger.warning(f"[MCPStorageClient] CallToolResult.content格式未知: {type(content)}")
                    ...
                    if isinstance(content, list) and len(content) > 0:
                        ...
                        if isinstance(first_item, dict) and "text" in first_item:
                            ...
                            try:
                                ...
                                # 如果不是JSON，直接使用
                                self.logger.warning("[MCPStorageClient] MCP响应不是JSON格式")
                                medical_data["raw_response"] = text_data
                
                # 如果直接包含医疗数据字段
                elif any(key in mcp_result for key in ["medical_history", "medications", "lab_results"]):
                    # ✅ 修复：检查是否有medical_data嵌套字段
                    if "medical_data" in mcp_result and isinstance(mcp_result["medical_data"], dict):
                ...
                if hasattr(mcp_result, '__dict__'):
                    try:
                        ...
                        if isinstance(obj_dict, dict):
                            ...
                            # 如果直接包含医疗数据字段
                            elif any(key in obj_dict for key in ["medical_history", "medications", "lab_results"]):
                                medical_data.update(obj_dict)
                            else:
                                medical_data["raw_response"] = obj_dict
                    except Exception as e:
                        self.logger.warning(f"[MCPStorageClient] 无法将对象转换为字典: {e}")
                
                if not medical_data.get("raw_response"):
                    self.logger.warning(f"[MCPStorageClient] 未知的MCP响应格式: {type(mcp_result)}")
                    # 尝试获取对象的字符串表示
                    try:
                        medical_data["raw_response"] = str(mcp_result)
                    except:
                        medical_data["raw_response"] = f"<{type(mcp_result).__name__} object>"
        ...
        if isinstance(medical_data, dict) and "medical_data" in medical_data and isinstance(medical_data["medical_data"], dict):
            nested_medical_data = medical_data["medical_data"]
            # 如果顶层字段为空或null，但嵌套字段有数据，则提取
            for key in ["medical_history", "medications", "lab_results", "imaging_data", "surgical_records", "health_monitoring"]:
                if (not medical_data.get(key) or medical_data[key] is None) and nested_medical_data.get(key):
                    medical_data[key] = nested_medical_data[key]
                    self.logger.info(f"[MCPStorageClient] ✅ 从medical_data嵌套字段中提取{key}")
            # 更新sources
        ...
        # 打印解析后的医疗数据详情（用于调试）
        self.logger.debug(f"[MCPStorageClient] 解析后的医疗数据键: {list(medical_data.keys())}")
    ...
    async def retrieve_medical_data(
        ...
        for (location, server_url), addresses in location_server_groups.items():
            task = self._retrieve_from_location_and_server(location, server_url, addresses, user_id)
            tasks.append(task)
    ...
    def _group_by_location_and_server(
        ...
        for addr in data_addresses:
            ...
            if not server_url:
                self.logger.warning(f"[MCPStorageClient] 数据地址中缺少server_url，使用环境变量默认值")
                # 使用环境变量中的默认值
                if location == "beijing":
            ...
            if not server_url:
                self.logger.error(f"[MCPStorageClient] 无法确定MCP服务器URL，跳过该数据地址: {addr}")
                continue
        ...
        self.logger.info(f"[MCPStorageClient] 按地域和服务器分组完成，共 {len(groups)} 个组合")
        for (loc, url), addrs in groups.items():
            self.logger.info(f"[MCPStorageClient]   - {loc} - {url}: {len(addrs)} 个数据地址")
    ...
    async def _retrieve_from_location_and_server(
        ...
        for addr in data_addresses:
            ...
            if token:
                ...
                self.logger.info(f"[MCPStorageClient] 从数据地址中提取到access_token: {token[:20]}... (长度: {len(token)})")
                print(f"[MCPStorageClient] ✅ 从数据地址中提取到access_token，将用于MCP服务器连接认证")
                print(f"  Token预览: {token[:30]}... (长度: {len(token)})")
                break
        
        # 如果没有提取到token，记录警告
        if not access_token_from_data:
            self.logger.warning(f"[MCPStorageClient] ⚠️  数据地址中未找到access_token")
            print(f"[MCPStorageClient] ⚠️  警告: 数据地址中未找到access_token")
            print(f"  数据地址详情:")
            for i, addr in enumerate(data_addresses, 1):
                print(f"    {i}. address: {addr.get('address', 'N/A')}")
                print(f"       data_type: {addr.get('data_type', 'N/A')}")
                print(f"       access_token: {'有' if addr.get('access_token') else '无'}")
            print(f"  将使用环境变量中的默认token（可能无效）")
        
        # 获取或创建MCP服务器连接
        mcp_server = self.mcp_servers.get(server_key)
        if not mcp_server:
            # 动态创建新的MCP服务器连接
            self.logger.info(f"[MCPStorageClient] 创建新的MCP服务器连接: {location} - {server_url}")
            print(f"[MCPStorageClient] 🔄 创建新的MCP服务器连接: {location} - {server_url}")
            ...
            print(f"[MCPStorageClient] ✅ 已创建MCP服务器连接: {location} - {server_url}")
        ...
        if access_token_from_data:
            ...
            if old_token != access_token_from_data:
                ...
                self.logger.info(f"[MCPStorageClient] 已使用数据代理应用返回的access_token更新MCP服务器配置: {server_url}")
                self.logger.info(f"[MCPStorageClient] 旧token: {old_token[:20] if old_token else 'N/A'}...")
                self.logger.info(f"[MCPStorageClient] 新token: {access_token_from_data[:20]}...")
                ...
                if mcp_server._initialized:
                    self.logger.info(f"[MCPStorageClient] MCP服务器已初始化，清理旧连接并重新初始化")
                    print(f"[MCPStorageClient] ⚠️  MCP服务器已初始化，清理旧连接并重新初始化（使用新token）")
                    try:
                ...
                self.logger.info(f"[MCPStorageClient] Token未变化，继续使用现有连接: {server_url}")
        elif not mcp_server.config.token or mcp_server.config.token == "test":
            self.logger.warning(f"[MCPStorageClient] ⚠️  数据地址中未找到access_token，且当前token为默认值: {server_url}")
            self.logger.warning(f"[MCPStorageClient] 可能无法通过MCP服务器认证")
            print(f"[MCPStorageClient] ⚠️  警告: 数据地址中未找到access_token，使用默认token可能无法通过认证: {server_url}")
        ...
        # 打印详细的请求信息
        print(f"\n" + "=" * 80)
        print(f"[MCP数据存储服务请求] 详细请求信息")
        print("=" * 80)
        print(f"  地域: {location}")
        print(f"  MCP服务器URL: {server_url} (配置: {mcp_server.config.server_url})")
        print(f"  MCP服务器Token: {mcp_server.config.token[:20] if mcp_server.config.token else 'N/A'}... (长度: {len(mcp_server.config.token) if mcp_server.config.token else 0})")
        print(f"  传输类型: {mcp_server.config.transport_type}")
        print(f"  工具名称: get_patient_medical_info")
        ...
        for i, addr in enumerate(data_addresses, 1):
            ...
            access_token = addr.get('access_token', '')
            if access_token:
                print(f"       access_token: {access_token[:20]}... (长度: {len(access_token)})")
            else:
                print(f"       access_token: (空)")
        print(f"\n  MCP工具调用参数（只传递基础参数）:")
        print(f"    - department: {arguments.get('department', 'N/A')}")
        print(f"    - user_id: {arguments.get('user_id', 'N/A')}")
        print(f"    - request_id: {arguments.get('request_id', 'N/A')}")
        print(f"\n  注意: MCP工具只接受基础参数（department、user_id、request_id）")
        print(f"  数据地址信息已通过 MCP_SERVER_TOKEN 和 server_url 传递")
        print(f"  数据地址详情（用于参考）:")
        for i, addr in enumerate(data_addresses, 1):
            print(f"    {i}. address: {addr.get('address', 'N/A')}")
            print(f"       data_type: {addr.get('data_type', 'N/A')}")
            ...
            access_token = addr.get('access_token', '')
            if access_token:
                print(f"       access_token: {access_token[:30]}... (长度: {len(access_token)})")
            else:
                print(f"       access_token: (空)")
        ...
        try:
            # 执行MCP工具
            result = await mcp_server.execute_tool("get_patient_medical_info", arguments)
            
            # 解析响应
            medical_data = self._parse_mcp_response(result)
            
            # 检查是否有认证错误
            has_auth_error = medical_data.get("error") == "认证失败" or "invalid token" in str(medical_data.get("error_message", "")).lower() or "invalid token" in str(medical_data.get("raw_response", "")).lower()
            
            # 打印详细的响应信息
            print(f"\n" + "=" * 80)
            print(f"[MCP数据存储服务响应] 详细响应信息")
            print("=" * 80)
            print(f"  地域: {location}")
            ...
            if isinstance(result, dict):
                ...
                if isinstance(result.content, list) and len(result.content) > 0:
                    ...
                    if hasattr(result.content[0], 'text'):
                        text_preview = str(result.content[0].text)[:100] if len(str(result.content[0].text)) > 100 else str(result.content[0].text)
                        print(f"  content[0].text预览: {text_preview}")
            
            if has_auth_error:
                print(f"\n  ⚠️  认证错误详情:")
                print(f"    - 错误类型: 认证失败")
                print(f"    - 错误消息: {medical_data.get('error_message', medical_data.get('raw_response', 'N/A'))}")
                print(f"    - 可能原因:")
                print(f"      1. access_token无效或已过期")
                print(f"      2. access_token格式不正确")
                print(f"      3. MCP服务器不接受该token")
                print(f"    - 建议:")
                print(f"      1. 检查数据地址中的access_token是否正确")
                print(f"      2. 联系MCP服务器管理员验证token有效性")
                print(f"      3. 确认token是否已过期")
            else:
                print(f"\n  医疗数据摘要:")
            print(f"    - 病史数据: {'有' if medical_data.get('medical_history') else '无'}")
            if medical_data.get('medical_history'):
                if isinstance(medical_data.get('medical_history'), list):
                    ...
                    print(f"      键: {list(medical_data.get('medical_history').keys())}")
            print(f"    - 用药记录: {'有' if medical_data.get('medications') else '无'}")
            ...
            print(f"    - 化验报告: {'有' if medical_data.get('lab_results') else '无'}")
            ...
            print(f"    - 影像数据: {'有' if medical_data.get('imaging_data') else '无'}")
            print(f"    - 手术记录: {'有' if medical_data.get('surgical_records') else '无'}")
                ...
                for key, value in medical_data.items():
                    if key == "raw_response":
                        ...
                        if hasattr(value, '__dict__'):
                            ...
                            try:
                                if hasattr(value.content, '__iter__') and not isinstance(value.content, str):
                                    clean_data[key] = [str(item) for item in value.content]
                                else:
                                    clean_data[key] = str(value.content)
                            except:
                                clean_data[key] = f"<{type(value).__name__} object>"
                        else:
                            clean_data[key] = str(value) if not isinstance(value, (dict, list, str, int, float, bool, type(None))) else value
                    else:
                ...
                if hasattr(result, 'content'):
                    try:
                        ...
                        if isinstance(result.content, list) and len(result.content) > 0:
                            print(f"content[0]类型: {type(result.content[0])}")
                            if hasattr(result.content[0], 'text'):
                                print(f"content[0].text预览: {str(result.content[0].text)[:200]}...")
            ...
            # 根据是否有错误记录日志
            if has_auth_error:
                self.logger.error(f"[MCPStorageClient] {location}地域数据读取失败: 认证错误")
            else:
    ...
    def _merge_results(self, results: List[Any]) -> Dict[str, Any]:
        ...
        for result in results:
            if isinstance(result, Exception):
                self.logger.warning(f"[MCPStorageClient] 读取结果包含异常: {str(result)}")
                continue
            ...
            # 合并医疗数据
            for key in ["medical_history", "medications", "lab_results", "imaging_data", "surgical_records"]:
                if result.get(key):
