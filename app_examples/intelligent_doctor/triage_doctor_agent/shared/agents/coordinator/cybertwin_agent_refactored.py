...
class CybertwinAgent(BaseAgent):
    ...
    def _init_specialists(self):
        ...
        【微服务拆分后】不再初始化本地智能体实例，改为配置HTTP客户端
        """
        # 【微服务拆分】使用HTTP客户端配置替代本地智能体实例
        self.specialist_service_configs = {
            "internal_medicine": {
                "beijing": {
                    "base_url": os.getenv("INTERNAL_MEDICINE_BEIJING_URL", "http://localhost:8002"),
                    "api_key": os.getenv("INTERNAL_MEDICINE_BEIJING_API_KEY", "test_api_key")
                },
                "shanghai": {
                    "base_url": os.getenv("INTERNAL_MEDICINE_SHANGHAI_URL", "http://localhost:8003"),
                    "api_key": os.getenv("INTERNAL_MEDICINE_SHANGHAI_API_KEY", "test_api_key")
                }
            },
            "surgical": {
                "beijing": {
                    "base_url": os.getenv("SURGICAL_BEIJING_URL", "http://localhost:8004"),
                    "api_key": os.getenv("SURGICAL_BEIJING_API_KEY", "test_api_key")
                },
                "shanghai": {
                    "base_url": os.getenv("SURGICAL_SHANGHAI_URL", "http://localhost:8005"),
                    "api_key": os.getenv("SURGICAL_SHANGHAI_API_KEY", "test_api_key")
                }
            }
        }
        
        # 保留specialists字典结构以兼容现有代码，但存储的是配置信息
        self.specialists = {
            "internal_medicine": {
                "beijing": self.specialist_service_configs["internal_medicine"]["beijing"],
                "shanghai": self.specialist_service_configs["internal_medicine"]["shanghai"]
            },
            "surgical": {
                "beijing": self.specialist_service_configs["surgical"]["beijing"],
                "shanghai": self.specialist_service_configs["surgical"]["shanghai"]
            }
        }
        
        self.logger.info(f"[{self.name}] 专科医生服务HTTP客户端配置初始化完成")
        self.logger.info(f"  - 内科: 北京({self.specialist_service_configs['internal_medicine']['beijing']['base_url']})、上海({self.specialist_service_configs['internal_medicine']['shanghai']['base_url']})")
        self.logger.info(f"  - 外科: 北京({self.specialist_service_configs['surgical']['beijing']['base_url']})、上海({self.specialist_service_configs['surgical']['shanghai']['base_url']})")
    ...
    def _init_demo_mode(self):
        ...
        try:
            ...
            # 【微服务拆分后】TwoRoundDiagnosisCoordinator和DefaultStrategyHandler需要适配HTTP调用
            # 暂时传递配置信息，内部会使用HTTP客户端
            self.two_round_coordinator = TwoRoundDiagnosisCoordinator(self.specialist_service_configs)
            self.default_handler = DefaultStrategyHandler(self.specialist_service_configs)
    ...
    def clear_memory(self, user_id: str):
        ...
        try:
            ...
            # 2. 【微服务拆分后】专科医生微服务独立管理对话记忆，这里不再需要清除
            # 如果需要清除专科医生的对话记忆，可以通过HTTP API调用
            self.logger.info(f"[{self.name}] 已清除用户 {user_id} 的主智能体对话记忆（专科医生微服务独立管理）")
        ...
        try:
            ...
            if self.config.demo_mode and self.config.demo_mode.enable_demo_mode and self.demo_detector:
                ...
                if is_diabetes:
                    ...
                    return await self._execute_diabetes_demo(user_input, user_id, user_info, diagnosis_logger=diagnosis_logger)
                else:
                    # 非糖尿病相关：使用默认策略
                    self.logger.info(f"[{self.name}] 非糖尿病相关内容，使用默认策略（快速诊断）")
                    return await self._execute_default_strategy(user_input, user_id, user_info, diagnosis_logger=diagnosis_logger)
            
            # 3. 原有流程（非演示模式）
            intent = await self._recognize_intent(user_input, diagnosis_logger=diagnosis_logger)
    ...
    async def _recognize_intent(self, user_input: str, diagnosis_logger=None) -> str:
        """
        识别用户意图（简化版）
        ...
        if not self.intent_recognition:
            ...
            # 记录意图识别日志
            if diagnosis_logger:
                await diagnosis_logger.log_intent_recognition(intent)
    ...
    async def _route_to_specialists_with_context(
        ...
        # 并行调用各地域的专科医生（使用HTTP客户端）
        tasks = []
        task_info_list = []  # 保存每个任务对应的信息：(location, specialty, addresses)
        for location, specialty, addresses in routing_results:
            # 获取服务配置
            service_config = self.specialist_service_configs[specialty][location]
            ...
            task = client.diagnose(
                ...
                metadata={
                    ...
                    "round": shared_context.round_number if hasattr(shared_context, 'round_number') else 1
                }
            )
            tasks.append(task)
            task_info_list.append((location, specialty, addresses))
        ...
        for i, result in enumerate(results):
            start_time = time.time() if diagnosis_logger else None
            if isinstance(result, Exception):
                ...
                if isinstance(result, dict):
                    ...
                    if "agent" not in result:
                        location, specialty, _ = routing_results[i]
                        result["agent"] = f"{specialty}_{location}"
                    valid_results.append(result)
                    
                    # 记录专科医生诊断结果
                    if diagnosis_logger:
                        location = result.get('location', task_info_list[i][0] if i < len(task_info_list) else 'unknown')
                        specialty = result.get('specialization', task_info_list[i][1] if i < len(task_info_list) else 'unknown')
                        duration = (time.time() - start_time) * 1000 if start_time else 0.0
                        
                        # 记录MCP交互（如果有数据地址）
                        addresses = task_info_list[i][2] if i < len(task_info_list) else []
                        if addresses:
                            addr = addresses[0]
                            mcp_info = result.get('mcp_interaction', {})
                            if mcp_info:
                                mcp_address = mcp_info.get('mcp_server_address', addr.get('address', 'N/A'))
                                mcp_token = mcp_info.get('token', addr.get('access_token', ''))
                            ...
                            await diagnosis_logger.log_mcp_interaction(
                                round_num=shared_context.round_number if hasattr(shared_context, 'round_number') else 1,
                                location=location,
                                specialty=specialty,
        ...
        # 记录诊断结果汇总
        if diagnosis_logger and valid_results:
    ...
    async def _execute_diabetes_demo(
        ...
        try:
            ...
            round1_results = await self.two_round_coordinator.first_round_diagnosis(
                ...
                user_info=user_info,  # 传递用户信息，包含userid、username、id_token
                diagnosis_logger=diagnosis_logger
            )
            ...
            if specialist_results:
                confidences = [r.get('confidence', 0.0) for r in specialist_results if isinstance(r, dict)]
                ...
                for r in specialist_results:
                    if isinstance(r, dict):
                        ...
                        if location:
                            data_sources.add(location)
            
            # 记录综合报告
            if diagnosis_logger:
                import time
                from datetime import datetime
                # 从diagnosis_logger的current_session获取开始时间
                if diagnosis_logger.current_session and 'start_time' in diagnosis_logger.current_session:
                    start_time = diagnosis_logger.current_session['start_time']
                    if isinstance(start_time, datetime):
                        duration = (datetime.now() - start_time).total_seconds()
                    else:
                        duration = time.time() - start_time if isinstance(start_time, (int, float)) else 0
                else:
                    duration = 0
                await diagnosis_logger.log_comprehensive_report(
            ...
            return self.name, {
                ...
                "specialist_results": specialist_results,
                "rounds": rounds,
                "data_sources": list(data_sources),  # 可以从 specialist_results 中提取
                "avg_confidence": avg_confidence,
                "duration": duration if diagnosis_logger else 0
            }
