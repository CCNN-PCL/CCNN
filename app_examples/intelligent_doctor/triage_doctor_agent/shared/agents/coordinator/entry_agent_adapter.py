...
class EntryAgentAdapter:
                            ...
                            if isinstance(status_message, dict):
                                ...
                                if parts and isinstance(parts, list):
                                    ...
                                    if error_texts:
                                        ...
                                        error_msg = f"数据代理应用返回错误: {result['message']}"
                                        self.logger.warning(f"[EntryAgentAdapter] {error_msg}")
                                        print(f"[EntryAgentAdapter] [错误] {error_msg}")
                                        
                                        # 记录错误到诊断日志（如果提供了diagnosis_logger）
                                        # 注意：这里无法直接访问diagnosis_logger，需要在调用invoke时传递
                                        # 但错误信息已经包含在result中，会在log_data_proxy_response中记录
                                else:
    ...
    async def invoke(self, request_data: Dict[str, Any], diagnosis_logger=None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=httpx.Timeout(self.timeout)) as httpx_client:
                ...
                try:
                    ...
                    # 记录A2A建链（Agent Card获取成功）
                    if diagnosis_logger:
                        try:
                            agent_card_dict = agent_card.model_dump() if hasattr(agent_card, 'model_dump') else {}
                            ...
                            self.logger.warning(f"记录A2A建链日志失败: {e}")
