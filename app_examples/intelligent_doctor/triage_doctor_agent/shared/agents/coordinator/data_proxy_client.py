...
class DataProxyClient:
    ...
    async def interact_with_context(
        ...
        while conversation_state.round < self.config.max_rounds:
            ...
            # 调用数据代理应用
            response = await self._call_data_proxy_app(request_data, diagnosis_logger=diagnosis_logger)
            ...
            if response.get("needs_more_info"):
            ...
            elif response.get("has_direct_data") and response.get("medical_data"):
                # 获取到直接医疗数据（无需数据地址）
                conversation_state.state = "completed"
                self.logger.info(f"[DataProxyClient] 获取到直接医疗数据，完成数据请求")
                # 直接医疗数据已经存储在 shared_context.direct_medical_data 中
                break
                ...
                # 其他情况：响应成功但没有数据地址和直接数据，可能是异常情况
                self.logger.warning(f"[DataProxyClient] 响应成功但没有数据地址或直接数据，可能已完成")
                if response.get("complete", True):  # 默认认为已完成
                    conversation_state.state = "completed"
                    break
    ...
    async def _call_data_proxy_app(self, request_data: Dict[str, Any], diagnosis_logger=None) -> Dict[str, Any]:
        """
        ...
        # 记录数据代理请求
        round_num = request_data.get('context', {}).get('conversation_round', 1)
        protocol = "EntryAgent" if self.config.use_entry_agent and self.entry_agent_adapter and ENTRY_AGENT_AVAILABLE else "HTTP"
        
        if diagnosis_logger:
            await diagnosis_logger.log_data_proxy_request(
                round_num=round_num,
        ...
        if self.config.use_entry_agent and self.entry_agent_adapter and ENTRY_AGENT_AVAILABLE:
            ...
            print()
            try:
                response = await self._call_via_entry_agent(request_data, diagnosis_logger=diagnosis_logger)
                # 记录数据代理响应
                if diagnosis_logger:
                    await diagnosis_logger.log_data_proxy_response(
                        round_num=round_num,
                ...
                error_msg = f"EntryAgent调用失败: {str(e)}"
                self.logger.warning(f"[DataProxyClient] {error_msg}")
                print(f"[DataProxyClient] ❌ {error_msg}")
                
                # 记录错误到诊断日志
                if diagnosis_logger:
                    await diagnosis_logger.log_error(error_msg, e)
        ...
        print()
        response = await self._call_via_http(request_data)
        # 记录数据代理响应
        if diagnosis_logger:
            await diagnosis_logger.log_data_proxy_response(
                round_num=round_num,
    ...
    async def _call_via_entry_agent(self, request_data: Dict[str, Any], diagnosis_logger=None) -> Dict[str, Any]:
        """通过标准A2A SDK的EntryAgent调用数据代理应用（使用prompt）"""
        round_num = request_data.get('context', {}).get('conversation_round', 1)
        self.logger.info(f"[DataProxyClient] 使用EntryAgent调用数据代理应用，轮次: {round_num}")
        
        # 记录A2A连接（在entry_agent_adapter中会记录Agent Card获取）
        if diagnosis_logger:
            await diagnosis_logger.log_a2a_connection(
                round_num=round_num,
                base_url=self.config.base_url,
                protocol="EntryAgent",
                token=self.config.token,
        ...
        response = await self.entry_agent_adapter.invoke(request_data, diagnosis_logger=diagnosis_logger)
