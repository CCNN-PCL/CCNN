...
# 从环境变量读取前端地址，支持Kubernetes部署时配置
medical_frontend = os.getenv("MEDICAL_FRONTEND_URL", "http://172.25.21.129:8089")

# OIDC提供者配置 - 从环境变量读取，支持Kubernetes部署时配置
OIDC_CONFIG = {
    'issuer': os.getenv("OIDC_ISSUER", "http://192.168.193.12:31111"),                           # cybertwin安全代理
    'client_id': os.getenv("OIDC_CLIENT_ID", "agent_doctor1"),
    'client_secret': os.getenv("OIDC_CLIENT_SECRET", "agent_doctor1-secret"),
    'redirect_uri': os.getenv("OIDC_REDIRECT_URI", "http://172.25.22.129:8001/oidc/callback"),         #本机回调地址
    'scope': os.getenv("OIDC_SCOPE", "openid profile email"),
    'post_logout_redirect_uri': os.getenv("OIDC_POST_LOGOUT_REDIRECT_URI", medical_frontend)             #登出回调地址
}
...
def get_jwks() -> dict:
    ...
    # 如果缓存存在且未过期，直接返回
    if _jwks is not None and _jwks_timestamp is not None and current_time - _jwks_timestamp < JWKS_CACHE_DURATION:
        return _jwks
...
# 从环境变量读取会话密钥，支持Kubernetes部署时配置（敏感信息应放在Secrets中）
SESSION_SECRET = os.getenv("OIDC_SESSION_SECRET", "oidc-rp-secret-key-32-bytes")
...
def oidc_callback(code: str, state: str, request: Request, response: Response):
    start_time = time.time()
    current_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time))
    logger.info(f"===========================回调认证开始 [{current_time_str}] ==============================================")
    ...
    try:
        ...
        logger.info(f"[时间记录] 开始换取令牌: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(token_start_time))}")
        ...
        logger.info(f"[时间记录] 令牌获取完成，耗时: {token_end_time - token_start_time:.3f}秒")
        ...
        logger.info(f"[时间记录] 开始验证ID令牌: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(verify_start_time))}")
        jwks = get_jwks()
        if not jwks:
        ...
        def custom_time_verification(payload):
            ...
            #adjusted_current_time = current_time - 28800   部署平台时不用减去28800
            ...
            if 'iat' in payload:
                ...
                if payload['iat'] > adjusted_current_time:
                    ...
                    raise jwt.InvalidTokenError(f"Token not yet valid by {time_diff} seconds")
            
            # 检查生效时间（nbf）
            if 'nbf' in payload:
                if payload['nbf'] > adjusted_current_time:
                    time_diff = payload['nbf'] - adjusted_current_time
                    raise jwt.InvalidTokenError(f"Token not active for {time_diff} seconds")
        ...
        logger.info(f"[时间记录] ID令牌验证完成，耗时: {verify_end_time - verify_start_time:.3f}秒")

        # 获取用户信息
        userinfo_start_time = time.time()
        logger.info(f"[时间记录] 开始获取用户信息: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(userinfo_start_time))}")
        ...
        userinfo_end_time = time.time()
        logger.info(f"[时间记录] 用户信息获取完成，耗时: {userinfo_end_time - userinfo_start_time:.3f}秒")
        global userinfo 
        ...
        logger.info(f"[时间记录] 回调认证完成 [{end_time_str}]，总耗时: {total_time:.3f}秒")
        logger.info("===========================回调认证结束==============================================")
        ...
        end_time = time.time()
        total_time = end_time - start_time
        logger.error(f"[时间记录] ID token过期，总耗时: {total_time:.3f}秒")
        return "ID token expired", 401
    except jwt.InvalidTokenError as e:
        end_time = time.time()
        total_time = end_time - start_time
        logger.error(f"[时间记录] Token验证失败: {str(e)}，总耗时: {total_time:.3f}秒")
        return f"Invalid token: {str(e)}", 401
    except Exception as e:
        end_time = time.time()
        total_time = end_time - start_time
        logger.error(f"[时间记录] 回调处理失败: {str(e)}，总耗时: {total_time:.3f}秒")
...
def oidc_me(request: Request):
    ...
    return { "userid": session["user_info"]["sub"], "username": session["user_info"]["name"],"token": session["id_token"]}
