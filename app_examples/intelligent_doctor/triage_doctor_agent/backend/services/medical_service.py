...
class MedicalService:
    ...
    async def upload_medical_image(self, user_id: str, hospital_id: str, image_data: bytes, 
        ...
        try:
            ...
            check_query = """
                ...
                WHERE user_id = %s AND hospital_id = %s AND image_type = %s AND examination_date = %s
            """
            cursor = db.cursor()
            ...
            query = """
                ...
                (user_id, hospital_id, image_data, image_type, image_category, examination_date, description, filename, file_size, created_at)
            ...
            image_id = cursor.fetchone()[0]  # 使用RETURNING获取ID
            db.commit()
            ...
            # 立即验证数据是否已插入
            verify_query = "SELECT id FROM medical_images WHERE id = %s AND user_id = %s"
    ...
    async def get_medical_images(self, user_id: str, hospital_id: str = None, 
        ...
        try:
            ...
            if hospital_id:
                where_conditions.append("hospital_id = %s")
                params.append(hospital_id)
            
            if image_type:
                where_conditions.append("image_type = %s")
            ...
            query = f"""
                SELECT id, user_id, hospital_id, image_data, image_type, image_category, 
                       examination_date, description, filename, file_size, created_at as timestamp
                FROM medical_images 
                WHERE {where_clause}
    ...
    async def delete_medical_image(self, image_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 先检查影像是否存在
            check_query = "SELECT id FROM medical_images WHERE id = %s AND user_id = %s"
            cursor = db.cursor()
            ...
            # 删除影像
            delete_query = "DELETE FROM medical_images WHERE id = %s AND user_id = %s"
    ...
    async def upload_medical_record(self, user_id: str, hospital_id: str, record_data: bytes,
        ...
        try:
            ...
            # 插入医疗记录到PostgreSQL
            query = """
                INSERT INTO medical_records (user_id, hospital_id, record_data, record_type, description, filename, file_size, created_at)
            ...
            cursor = db.cursor()
            cursor.execute(query, (
                user_id, hospital_id, medical_history, record_type, 
                description, filename, len(record_data), datetime.now()
            ))
