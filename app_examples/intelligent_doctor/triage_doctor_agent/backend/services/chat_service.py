...
class ChatService:
    ...
    async def handle_chat(self, user_input: str, user_id: str, user_info: Dict[str, Any], context: Optional[Dict[str, Any]] = None, diagnosis_logger=None) -> Optional[Dict[str, Any]]:
        """
        处理聊天消息
        ...
        try:
            ...
            logger.info(f"[聊天服务] 开始调用CybertwinAgent.execute()（重构版）")
            # 尝试通过**kwargs传递diagnosis_logger（解决inspect无法识别参数的问题）
            try:
                agent_name, result = await self.cybertwin_agent.execute(
                    user_input, user_id, user_info, 
                    **({"diagnosis_logger": diagnosis_logger} if diagnosis_logger else {})
                )
            except TypeError:
                # 如果失败，尝试不使用diagnosis_logger
                logger.warning("[聊天服务] diagnosis_logger参数传递失败，继续执行但不记录日志")
                agent_name, result = await self.cybertwin_agent.execute(user_input, user_id, user_info)
            print(f"[OK] CybertwinAgent（重构版）处理完成！")
            ...
            if result and isinstance(result, dict):
                ...
                if result.get("status") == "success" and result.get("report"):
                    ...
                    logger.info(f"[聊天服务] 诊断成功，报告长度: {len(report)}, 专科医生数: {len(specialist_results)}, 轮次: {rounds}")
                    
                    # 清理metadata中的异常对象，确保可以序列化
                    cleaned_specialist_results = self._clean_serializable_data(specialist_results)
                    ...
                    await self._store_chat_message(user_id, "assistant", report, user_info)
                    ...
                    # 存储错误记录
                    await self._store_chat_message(user_id, "user", user_input, user_info)
                    await self._store_chat_message(user_id, "assistant", f"诊断失败: {error_msg}", user_info)
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                if isinstance(value, (Exception, asyncio.CancelledError)):
                    logger.warning(f"[ChatService] 跳过无法序列化的异常对象: {type(value).__name__} at key '{key}'")
                    continue
            ...
            if hasattr(db.db_manager, 'pool'):
                ...
                async with db.db_manager.pool.get_connection() as conn:
                    async with conn.cursor() as cursor:
                        await cursor.execute("DELETE FROM chat_history WHERE user_id = %s", (user_id,))
                        await conn.commit()
    ...
    def clear_dialogue_memory(self, user_id: str) -> bool:
        ...
        try:
            if self.cybertwin_agent and hasattr(self.cybertwin_agent, 'clear_memory'):
                ...
                logger.warning(f"[聊天服务] CybertwinAgent未初始化或没有clear_memory方法")
                return False
        except Exception as e:
    ...
    async def _store_chat_message(self, user_id: str, role: str, content: str, user_info: Optional[Dict[str, Any]] = None) -> bool:
        """
            ...
            if hasattr(db.db_manager, 'store_chat_message'):
                ...
                if hasattr(db.db_manager, 'get_user'):
                    ...
                    if not existing_user:
                        # 用户不存在，尝试创建用户
                        logger.info(f"[存储聊天消息] 用户 {user_id} 不存在，尝试创建用户记录")
                        if user_info:
                            username = user_info.get('username', user_info.get('name', user_id))
                            email = user_info.get('email', '')
                            # 对于OIDC用户，使用占位符密码哈希
                            password_hash = user_info.get('password_hash', 'oidc_user_no_password')
                        else:
                            # 如果没有user_info，使用默认值
                            username = user_id
                            email = ''
                        ...
                        if hasattr(db.db_manager, 'create_user'):
                            ...
                            if success:
                                ...
                                logger.warning(f"[存储聊天消息] 用户 {user_id} 创建失败，但继续尝试存储消息")
                        else:
                            logger.warning(f"[存储聊天消息] 数据库管理器不支持创建用户，跳过用户创建")
